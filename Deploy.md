...
放通范围见 [§3.3](#33-服务器-cminio预生产)：9000 需允许**服务器 B** 与**用户浏览器**（或经公网 endpoint）；9001 建议仅运维 IP。
...
#### 5.1.5 CORS（浏览器从服务器 A 域名直传）

presigned URL 直传时，浏览器会跨域访问 **服务器 C 的 9000 端口**，必须配置 bucket CORS：
...
mc cors set local/xjicloud /tmp/cors.json
mc cors list local/xjicloud
```
