...
import { prefersReducedMotion } from '@/presentation/motion'
...
const backdropClass = computed(() => ({
  'app-sheet-backdrop': true,
  'app-sheet-backdrop--popover': effectiveMode.value === 'popover',
  'app-sheet-backdrop--sheet': effectiveMode.value === 'sheet',
}))

const panelClass = computed(() => ({
  'app-sheet__panel': true,
  'app-sheet__panel--popover': effectiveMode.value === 'popover',
  'app-sheet__panel--sheet': effectiveMode.value === 'sheet',
}))
...
const transitionName = computed(() => {
  ...
  return effectiveMode.value === 'popover' ? 'app-popover' : 'app-sheet'
})

const titleId = computed(() => props.labelledBy ?? `app-sheet-title-${titleSuffix}`)
    ...
    <Transition :name="transitionName">
      ...
      >
        <div
          ...
          @click.stop
        >
          <div v-if="title || $slots.header" class="app-sheet__header">
            <slot name="header">
              <h2 :id="titleId" class="app-sheet__title">{{ title }}</h2>
            </slot>
          </div>
