...
const defaultFiles = [
  path.resolve(path.dirname(new URL(import.meta.url).pathname), '../../../vendors/supersplat/index.js'),
  path.resolve(path.dirname(new URL(import.meta.url).pathname), '../public/supersplat/index.js'),
]
...
for (const file of files) {
  ...
  if (s.includes(withExport)) {
    console.log('already has export-ply', file)
    continue
  }
  if (s.includes(importOnlyBroken)) {
    s = s.replace(importOnlyBroken, withExport)
    fs.writeFileSync(file, s)
    console.log('patched broken-import → export-ply', file)
    continue
  }
  if (s.includes(importOnly)) {
    s = s.replace(importOnly, withExport)
    fs.writeFileSync(file, s)
    console.log('patched import-local → export-ply', file)
    continue
  }
  if (s.includes(dirtyOnly)) {
    s = s.replace(dirtyOnly, withExport)
    fs.writeFileSync(file, s)
    console.log('patched dirty-only → export-ply', file)
    continue
  }
